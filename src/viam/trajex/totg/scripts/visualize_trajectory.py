...
def plot_phase_plane(data, ax):
    ...
    # Otherwise plot the single combined min(joint, TCP) curve.
    if has_tcp_components:
        _plot_segmented(ax, s_ip, joint_vel, color='orange', linewidth=1.5,
                        label='Joint Velocity Limit', zorder=2)
        if tcp_vel is not None and any(v is not None for v in tcp_vel):
            _plot_segmented(ax, s_ip, tcp_vel, color='purple', linewidth=1.5,
    ...
    if lcs_gap_s:
        ...
        if gap_has_components:
            for seg_s, seg_v in build_gap_segments(lcs_gap_joint):
                ax.plot(seg_s, seg_v, color='orange', linestyle='--', linewidth=1.5, alpha=0.6, zorder=2)
            for seg_s, seg_v in build_gap_segments(lcs_gap_tcp):
                ax.plot(seg_s, seg_v, color='purple', linestyle='--', linewidth=1.5, alpha=0.6, zorder=2)
        else:
